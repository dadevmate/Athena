...
struct GeogView: View {
    ...
    var body: some View {
        NavigationView {
                ...
                HStack {
                    ...
                    Button("Ask question") {
                        if topic.trimmingCharacters(in: .whitespaces) != "" {
                            text = "I have a geography question: '\(qn)'. This is of the topic '\(topic)'. Give an answer suitable of the \(select) level"
                            send()
                        }
                    }
                    Spacer()
            ...
            .navigationTitle("Geography help")
            .toolbar {
...
struct GeogView_Previews: PreviewProvider {
